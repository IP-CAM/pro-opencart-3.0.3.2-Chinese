...
class ModelLocalisationCity extends Model
{
    ...
    {
        $this->db->query("INSERT INTO " . DB_PREFIX . "city SET name = '" . $this->db->escape($data['name']) . "', parent_city_id='" . (int)$data['parent_id'] . "', zone_id = '" . (int)$data['zone_id'] . "', status = '" . (int)$data['status'] . "'");
    ...
    {
        $this->db->query("UPDATE " . DB_PREFIX . "city SET name = '" . $this->db->escape($data['name']) . "', parent_city_id='" . (int)$data['parent_id'] . "', zone_id = '" . (int)$data['zone_id'] . "', status = '" . (int)$data['status'] . "' WHERE city_id = '" . (int)$city_id . "'");
    ...
    public function getCitiesByZoneId($zoneId, $topLevel = true)
    {
        ...
        if (!$city_data) {
            if ($topLevel) {
                $sql = "SELECT * FROM " . DB_PREFIX . "city WHERE zone_id = '" . (int)$zoneId . "' AND status = '1' AND parent_city_id=0 ORDER BY name";
            } else {
                $sql = "SELECT * FROM " . DB_PREFIX . "city WHERE parent_city_id = '" . (int)$zoneId . "' AND status = '1' ORDER BY name";
            }
